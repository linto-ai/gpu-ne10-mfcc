...
{
    Record_Manager *manager = new Record_Manager("test.raw",false,"meeting","mfcc.out",800,400,13);
    MFCC *mfcc = new MFCC(400,160,povey); // MFCC window size and sliding part
    AudioParameter audioParams;
        audioParams.sampleRate = 16000;
        audioParams.channels = 1;
        audioParams.chunkSize = 2*400-160; // 2 windows for MFCC less sliding part
    AudioInput* input = new AudioInput(&audioParams);
    // Link modules
    manager->mfcc_on = true;
